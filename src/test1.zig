...
test "coerse from a subset to superset" {
...
test "error union" {
    ...
    const no_error = maybe_error catch 10;
...
test "try" {
    ...
    try expect(v == 12); // this is never reached
}
...
test "inferred error set" {
    ...
    // we must unwrap it via "try", "catch", or "if"
    _ = x catch {};
}
...
test "set enum ordinal value" {
    ...
    try expect(@intFromEnum(Value2.foobar) == 1000001);
}
...
//     result.float = 12.34; // no go!
// }

const Tag = enum { a, b, c };
const Tagged = union(Tag) { a: u8, b: f32, c: bool };

test "swith on tagged union" {
...
test "if optional paylload capture" {
    const a: ?i32 = 5;
    if (a != null) {
        const value = a.?;
        _ = value;
...
test "branching on types" {
    const a = 11;
    const b: if (a < 10) f32 else i32 = 6;
    try expect(b == 6);
